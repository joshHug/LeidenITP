...
In this exercise, you will practise with the use of sets. A set can be created similar to a list only difference is that sets use curly brackets `{...}` instead of square brackets `[...]`. A programming set is comparable to a mathematical set. There is no order (no index), it contains unique items, and it stores several objects. Another way to look at sets, is as a dictionary without values. For example, the set with the integers 1 and 2 can be created as follows: `{1, 2}`. **Important note**, an empty set can not be created with curly brackets `{}` as that syntax is already reserved for empty dictionaries. However, similar to other datastructures such as lists and dictionaries it can be created using `set()` with no argument.

Open exercise `sets.py`, follow the instructions and complete the functions. 
